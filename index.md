...
<p align="center"><img src="https://raw.githubusercontent.com/BurchallCooper/CS7641-Project/gh-pages/System.png" alt="system drawing" height="450" width="450" /></p>
...
The laser detection worked well indoors with varied lighting conditions.  Some testing was done with outdoor lighting conditions.  It was noted that outdoor bright sunlight has significant green spectral preventing this approach from working well.  However, green lasers were all that were readily available, and worked well indoors for this proof of concept. 

It was also found that pose detection required the human target to occupy at least 25% of the frame in either the x or y dimension.  This limited range of the current system to under 4 meters.  The architecture of the targeting system will need to be modified to incorporate a preprossing stage to solve this problem.  This preprocessor would need to detect the target and zoom the image for the pose detector to work optimally.  With only electronic zoom, the distance could be increased to 20 meters with the existing cameras.  Optical zoom would be needed for long distances.
...
Testing against COCO [5] was more problematic.  If there was a person present in the image, the pose detector worked well in providing the pose estimate.  But, in scenes with no humans present, as illustrated in figure 8, the neural network occasionally misclassified an inanimate object as human and provided a pose estimate.  In images with an animal present, and no human presemt, the network provided a human pose estimate for the animal.  The pose estimate for the rooster, also shown in figure 8, illustrates this problem.  
...
[last edits to this document were made by William B. Cooper 😉]
